...
class User():
    ...
    def initialize_user_authentication(self, user_type, username, password, firstname=None, lastname=None, userrole=None, business_name=None, business_address=None):
        if user_type == "new":
            self.authenticate_new_user(username, password, firstname, lastname, userrole, business_name, business_address)
        elif user_type == "existing":
            self.authenticate_existing_user(username, password)
        self.update_session_state()

    def authenticate_new_user(self, username, password, firstname, lastname, userrole, business_name, business_address):
        self.thread_metadata= {"username": username, "password": password, "firstname": firstname, "lastname": lastname, "fullname": f"{firstname} {lastname}", "userrole": userrole, "createddate": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "business_name": business_name, "business_address": business_address}
        ...
        new_user_data = {
            ...
            self.auth_settings['businessname_column']: self.user_data['business_name'],
            self.auth_settings['businessaddress_column']: self.user_data['business_address']
        }
    ...
    def update_session_state(self):
        ...
        self.business_name = self.user_data['business_name']
        self.business_address = self.user_data['business_address']
        st.session_state.user.update({
            ...
            'business_name': self.user_data['business_name'],
            'business_address': self.user_data['business_address'],
        })       
...
class UserFlow():
            ...
            with userauth2_container:
                ...
                with userauth_columns[1]:
                    ...
                    if st.session_state.user['user_auth_type'] == "new":
                        ...
                        st.text_input(label="Business Name", key="business_name")
                        st.text_input(label="Business Address", key="business_address")
                        st.radio(
    ...
    def userflow2_userauth_callback(self, user_type, username, password, firstname=None, lastname=None, userrole=None):
        ...
        st.session_state.user['business_name'] = self.user.user_data['business_name']
        st.session_state.user['business_address'] = self.user.user_data['business_address']
